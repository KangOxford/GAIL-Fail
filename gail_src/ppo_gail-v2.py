...
if generating_experts:
    expert = SAC(policy="MlpPolicy", 
                  env = env, 
                  verbose=1,
                  tensorboard_log="~/GAIL-Fail/tensorboard/debug_sac_walker2dv0_expert/")
    expert.learn(1e5,tb_log_name="sac_seal_run") 
    expert.save("debug_sac_seal_expert_v5")
else: 
    expert = SAC.load("~/GAIL-Fail/experts/sac_seal_expert_1.zip")
