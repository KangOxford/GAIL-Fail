...
def default_xy_fn(r):
    ...
    y = smooth(r.monitor.r, radius=1)
    return x,y 

def evaluate_mean_returns(r):
    x = r.progress.TotalEnvInteracts
    y = smooth(r.progress.EvalEpRetMean, radius=1)
    return x,y 

def length_fn(r):
    x = np.cumsum(r.monitor.l)
    y = smooth(r.monitor.l, radius=1)
    return x,y 

def bc_evaluation_xy_fn(r):
    x = r.progress.IterSoFar
    y = smooth(r.progress.EpRewMean, radius=1)
    return x, y

def tmp_evaluation_xy_fn(r):
    x = r.progress.TotalEnvInteracts
    y = smooth(r.progress.TestEpRet, radius=1)
    return x, y
...
def default_split_fn(r):
    ...
    match = re.search(r'[^/_]+(?=(_\d+)?\Z)', r.dirname)
    if match:
        return match.group(0)

def select_split_fn(r, default_str=''):
    import re
    # match name between slash and _<digits> at the end of the string
    # (slash in the beginning or -<digits> in the end or either may be missing)
    match = re.search(default_str, r.dirname)
    if match:
        return match.group(0)
